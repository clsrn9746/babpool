package com.babpool.one.handler;
...
@Component
public class HandlerOne extends TextWebSocketHandler {
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		case "CMD_ENTER":
			...
			map.put("marketNo", mapReceive.get("marketNo"));
			sessionList.add(map);
			
			// 같은 채팅방에 입장 메세지 전송
			for (int i = 0; i < sessionList.size(); i++) {
				Map<String, Object> mapSessionList = sessionList.get(i);
				String bang_id = (String) mapSessionList.get("marketNo");
				WebSocketSession sess = (WebSocketSession) mapSessionList.get("session");
				
				if(bang_id.equals(mapReceive.get("marketNo"))) {
		...
		case "CMD_MSG_SEND":
			...
			for (int i = 0; i < sessionList.size(); i++) {
				...
				if (bang_id.equals(mapReceive.get("marketNo"))) {
					...
					mapToSend.put("marketNo", mapReceive.get("marketNo"));
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		for (int i = 0; i < sessionList.size(); i++) {
			...
			String bang_id = (String) map.get("marketNo");
